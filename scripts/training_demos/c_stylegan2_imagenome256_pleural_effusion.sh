...
if [[ $# -lt 1 ]]; then
    ...
    echo
    echo "    - Resolution: 256"
    echo "    - Conditioning on: pleural effusion (~17% of total images), single binary condition"
    echo "    - Views: PA"
    echo "    - Squaring: resized"
    echo "    - Balance: slight imbalance (pleural effusion: ~17.8%)."
    echo
...
TRAIN_ANNOTATIONS=/home/gregschuit/projects/data/annotations/pleural_effusion_PA_train.txt
VALID_ANNOTATIONS=/home/gregschuit/projects/data/annotations/pleural_effusion_PA_valid.txt

./scripts/dist_train.sh ${GPUS} stylegan2 \
    --job_name='c_stylegan2_imagenome256_pleural_effusion_pa_resized' \
    --seed=0 \
    --resolution=256 \
    --image_channels=1 \
