...
if [[ $# -lt 1 ]]; then
    ...
    echo
    echo "    - Resolution: 256"
    echo "    - Conditioning on: lung opacity (~48% of total images), single binary condition"
    echo "    - Views: PA"
    echo "    - Squaring: resized"
    echo "    - Balance: balanced by default, as ~48% comes with lung opacity."
    echo
...
TRAIN_ANNOTATIONS=/home/gregschuit/projects/data/annotations/lung_opacity_PA_train.txt
VALID_ANNOTATIONS=/home/gregschuit/projects/data/annotations/lung_opacity_PA_valid.txt

./scripts/dist_train.sh ${GPUS} stylegan2 \
    --job_name='c_stylegan2_imagenome256_lung_opacity_pa_resized_mirror=false' \
    --seed=0 \
    --resolution=256 \
    --image_channels=1 \
