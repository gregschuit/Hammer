...
class FIDMetric(BaseGANMetric):
    ...
    def extract_fake_features(self, generator, generator_kwargs):
        ...
        # check if dir exists
        if not os.path.exists('batch_codes'):
            os.makedirs('batch_codes')
        if not os.path.exists('batch_labels'):
            os.makedirs('batch_labels')
        ...
        all_features = []
        for batch_number, start in enumerate(range(0, self.replica_latent_num, batch_size)):
            ...
            with torch.no_grad():
                ...
                torch.save(batch_codes, f'batch_codes/batch_{self.replica_latent_num}_{batch_number}.pt')
                torch.save(batch_labels, f'batch_labels/batch_{self.replica_latent_num}_{batch_number}.pt')
                torch.save(batch_images, f'batch_images/batch_{self.replica_latent_num}_{batch_number}.pt')
