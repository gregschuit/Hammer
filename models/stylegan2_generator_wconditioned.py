...
class StyleGAN2GeneratorW(nn.Module):
    ...
    def __init__(self,
        ...
        w_dim_plus_label = w_dim + 1

        # This is used for truncation trick.
        if self.repeat_w:
            self.register_buffer('w_avg', torch.zeros(w_dim_plus_label))
        else:
            self.register_buffer('w_avg', torch.zeros(self.num_layers * w_dim_plus_label))

        self.synthesis = SynthesisNetwork(resolution=resolution,
                                          init_res=init_res,
                                          w_dim=w_dim_plus_label,
    ...
    def forward(self,
        ...
        """
        assert (z is not None) or (w is not None)
        batch_size = z.shape[0] if z is not None else w.shape[0]
        device = z.device if z is not None else w.device

        mapping_results = self.mapping(z=z, label=label, impl=impl, w=w)
        ...
        if self.training and w_moving_decay is not None:
            ...
            new = batch_w_avg.lerp(self.w_avg, w_moving_decay)
            self.w_avg.copy_(new)

        wp = mapping_results.pop('wp')
        if self.training and style_mixing_prob is not None:
            if np.random.uniform() < style_mixing_prob:
                new_z = torch.randn((batch_size, self.z_dim), device=device, dtype=torch.float32)
...
class MappingNetwork(nn.Module):
    ...
    def forward(self, z=None, label=None, impl='cuda', w=None):
        assert (z is not None) or (w is not None)
        ...
        if self.label_dim > 0:
            binary_label = label.argmax(dim=1).reshape(-1, 1)
            w = torch.cat((w, binary_label), dim=1)
